import ISliderClassNames from './interfaces/view/ISliderClassNames';

const sliderClassNames: ISliderClassNames = {
  slider: {
    main: 'simpleSlider',
    horizontal: 'simpleSlider_horizontal',
    vertical: 'simpleSlider_vertical',
  },
  bar: {
    main: 'simpleSlider__bar',
    horizontal: 'simpleSlider__bar_horizontal',
    vertical: 'simpleSlider__bar_vertical',
  },
  range: {
    main: 'simpleSlider__range',
    horizontal: 'simpleSlider__range_horizontal',
    vertical: 'simpleSlider__range_vertical',
  },
  toggle: {
    main: 'simpleSlider__toggle',
    horizontal: 'simpleSlider__toggle_horizontal',
    vertical: 'simpleSlider__toggle_vertical',
    active: 'simpleSlider__toggle_last-active',
  },
  thumb: {
    main: 'simpleSlider__thumb',
    horizontal: 'simpleSlider__thumb_horizontal',
    vertical: 'simpleSlider__thumb_vertical',
  },
  scale: {
    main: 'simpleSlider__scale',
    horizontal: 'simpleSlider__scale_horizontal',
    vertical: 'simpleSlider__scale_vertical',
  },
  scaleItem: {
    main: 'simpleSlider__scale-item',
    horizontal: 'simpleSlider__scale-item_horizontal',
    vertical: 'simpleSlider__scale-item_vertical',
  },
};

export default sliderClassNames;
